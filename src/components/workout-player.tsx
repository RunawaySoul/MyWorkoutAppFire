...
export function WorkoutPlayer({ initialAppData, workoutId }: WorkoutPlayerProps) {
  ...
  const saveProgress = useCallback(async () => {
    if (!currentLog || currentLog.status !== 'in-progress') return;
    const logToSave = { ...currentLog, currentExerciseIndex };
    const dataToSave = { ...appData, workoutLogs: appData.workoutLogs.map(l => l.id === logToSave.id ? logToSave : l)};
    await saveAppData(dataToSave);
  }, [appData, currentLog, currentExerciseIndex]);
  
  useEffect(() => {
    async function initializeLog() {
        let log = initialAppData.workoutLogs.find(l => l.workoutId === workoutId && l.status === 'in-progress');
        ...
        if (log) {
            ...
            const workoutToStart = initialAppData.workouts.find(w => w.id === workoutId);
            if (workoutToStart) {
                ...
                const newData = { ...initialAppData, workoutLogs: [...initialAppData.workoutLogs, newLog] };
  ...
  const updateLog = useCallback((updates: Partial<WorkoutLog>) => {
    setCurrentLog(prevLog => {
        ...
        setAppData(prevData => {
            const updatedLogs = prevData.workoutLogs.map(log => log.id === updatedLog.id ? updatedLog : log);
            return { ...prevData, workoutLogs: updatedLogs };
        });
        return updatedLog;
    });
  }, []);
  ...
  if (!isDataLoaded || !workout || !currentLog) {
